...
import { moveImage } from '../../../../redux/admin/admin.actions';
...
class ManageAssets extends React.Component {
  onSortEnd = ({ oldIndex, newIndex, collection }) => {
    if (oldIndex !== newIndex) {
      this.props.moveImage(oldIndex + 1, newIndex + 1, collection.userId);
    }
  };
  render() {
    return (
      <div className='flex flex-col justify-center '>
        <div className='w-full cus-bg-blue text-center text-2xl mb-6 text-gray-400 flex flex-col justify-start'>
          ...
          <SortableImagesContainer
            ...
            distance={1}
          >
            {this.props.user.images.map((image, i) => (
              <SortableImage
                index={i}
                key={i}
                image={image}
...
const mapDispatchToProps = (dispatch) => ({
  ...
  moveImage: (oldPos, newPos, id) => dispatch(moveImage(oldPos, newPos, id)),
});
